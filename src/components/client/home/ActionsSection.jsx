    ...
    <div className="w-full max-w-7xl mx-auto px-4 py-12 bg-gray-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {actionCards.map((card, index) => (
          <div
            key={index}
            className={`relative bg-white rounded-2xl p-8 border-2 border-dashed ${card.borderColor} text-center hover:shadow-lg transition-shadow duration-300 overflow-hidden`}
          >
            {/* Icon */}
            ...
            {/* Button */}
            <button className={`${card.buttonColor} text-white text-[16px] font-semibold py-3 px-6 rounded-full transition-colors duration-200 flex items-center justify-center mx-auto`}>
              {card.buttonText}
