...
const Footer = () => {
  ...
  const socialLinks = [
    {
      ...
      url: "https://www.instagram.com/wovegift/",
      label: "Instagram",
    },
    {
      icon: (
      ...
      url: "https://www.facebook.com/wovegift",
      label: "Facebook",
    },
    {
      icon: (
      ...
      url: "https://www.linkedin.com/company/wovegift-com/",
      label: "LinkedIn",
    },
  ];
  ...
  return (
    <footer className="bg-gradient-to-b from-[#FEF8F6] to-[#FDF7F8] text-gray-800">
      <div className="max-w-360 mx-auto pt-12 pb-8 px-6">

        {/* Top grid: branding full-width on mobile, then 4-col on desktop */}
        <div className="grid grid-cols-1 md:grid-cols-[1.5fr_1fr_1fr_1.5fr] gap-8 md:gap-12 mb-8">

          {/* Branding Section */}
          <div className="flex flex-col items-center md:items-start space-y-3">
            <div className="w-36 mb-2">
              <img src="/wovelogo.png" alt="Wove Gift" className="w-full h-full" />
            </div>
            <h4 className="font-semibold text-[#1A1A1A] text-[17px] fontPoppins text-center md:text-left">
              South Africa's #1 Gift Card Platform
            </h4>
            <p className="text-gray-600 text-sm leading-relaxed text-center md:text-left max-w-xs">
            ...
            <div className="flex gap-3 pt-2">
              {socialLinks.map(({ icon, url, label }, index) => (
                <div
                  key={index}
                  className="w-10 h-10 bg-[linear-gradient(114.06deg,#ED457D_11.36%,#FA8F42_90.28%)] flex items-center justify-center transition-all duration-300 rounded-[10px]"
                >
                  <a
                    href={url}
                    target="_blank"
                    ...
                    aria-label={label}
                    className="w-[38px] h-[38px] rounded-[10px] bg-white flex items-center justify-center"
                  >
          ...
          {/* Quick Links + Legal — side by side on mobile, separate on desktop */}
          <div className="grid grid-cols-2 gap-4 md:contents">

            {/* Quick Links */}
            <div>
              <h4 className="font-bold text-[#1A1A1A] mb-4 text-[14px] tracking-wide uppercase">
                Quick Links
              </h4>
              <ul className="space-y-3">
                {links.map((item, idx) => (
                  <li key={idx}>
                    <a
                      href={item.path}
                      className="text-[#4A4A4A] text-[14px] md:text-[16px] hover:text-pink-500 transition-colors duration-200"
                    >
                      {item.name}
                    </a>
                  </li>
          ...
          <div className="flex flex-col items-center md:items-start">
            <h4 className="font-bold text-[#1A1A1A] mb-3 text-[14px] md:text-[16px] tracking-wide uppercase">
              Stay In The Loop
            </h4>
            <p className="text-[#4A4A4A] text-[14px] md:text-[16px] font-medium mb-4 leading-relaxed text-center md:text-left max-w-sm">
              Get updates, special gifting moments, and announcements
            </p>
            <form onSubmit={handleSubscribe} className="flex flex-col gap-3 w-full items-center md:items-start">
              <input
                type="email"
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="group bg-[linear-gradient(114.06deg,#ED457D_11.36%,#FA8F42_90.28%)] text-white px-6 py-2.5 rounded-full text-sm font-semibold hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 w-fit self-center md:self-start cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed"
              >
              ...
              {feedback.message && (
                <p
                  className={`text-sm text-center md:text-left ${feedback.type === "success" ? "text-green-600" : "text-red-500"}`}
                  role="status"
                >
                  {feedback.message}
                </p>
        ...
        {/* Copyright — flush inside the same footer, no gap */}
        <div className="border-t border-gray-200 pt-5 text-center">
          <p className="text-gray-600 text-sm">
            © {currentYear ? `${currentYear} ` : ""}Wove Gift (Powered by MyPerks). All rights reserved.
          </p>
        </div>
      </div>
    </footer>
