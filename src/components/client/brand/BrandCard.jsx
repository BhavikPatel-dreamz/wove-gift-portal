...
const BrandCard = ({ 
  ...
  return (
    ...
    >
      ...
      >
        <Heart size={16} className={isFavorited ? 'text-red-500 fill-current' : 'text-gray-400'} />
      </button>
      ...
      {/* Choose Brand Button - appears on hover */}
      <div className="absolute inset-0 bg-black/20 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
        <button
          ...
          className="bg-white text-gray-800 px-6 py-2 rounded-lg font-semibold shadow-lg hover:bg-gray-50 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 ease-out"
        >
          Choose Brand
        </button>
      </div>

      <div className="flex justify-center mb-4 mt-4 group-hover:scale-110 transition-transform duration-300">
