...
const BrandManager = ({
  ...
  }, [initialBrands, initialPagination, initialStatistics, initialCategoryStats, initialReviewData]);
  ...
  const pendingStoreByBrandId = React.useMemo(() => {
    const map = {};
    (reviewData?.pending || []).forEach((store) => {
      if (store.brand?.id) {
        map[store.brand.id] = store;
      }
    });
    return map;
  }, [reviewData]);
  ...
  const createFormData = (brandData, isUpdate = false, existingBrand = null) => {
    ...
    if (isUpdate && existingBrand) formDataToSend.append("id", existingBrand.id);
    ...
    if (brandData.logo && brandData.logo instanceof File) formDataToSend.append("logo", brandData.logo);
  ...
  const toggleFeatured = async (id) => {
    ...
    try {
      setActionLoading(true);
      const formDataToSend = createFormData({ ...brand, isFeature: !brand.isFeature, logo: null }, true, brand);
  ...
  const toggleActive = async (id) => {
    ...
    try {
      setActionLoading(true);
      const formDataToSend = createFormData({ ...brand, isActive: !brand.isActive, logo: null }, true, brand);
      const result = await updateBrand(formDataToSend);
      if (result.success) {
  ...
  const handleEditClick = (brand) => router.push(`/brandsPartner/edit/${brand.id}`);
  const handleReviewClick = (brand) => router.push(`/brandsPartner/edit/${brand.id}?mode=review`);
  const redirectToSettlement = (brandId) => router.push(`/brandsPartner/${brandId}/settlements`);

  const handleApproveStore = async (shop, brandId) => {
    try {
      setApprovingShop(shop);
      const fd = new FormData();
      fd.append("shop", shop);
      await approveShopInstallationAction(fd);
      toast.success("Store approved successfully");
      router.push(`/brandsPartner/edit/${brandId}`);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 text-black">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between flex-col md:flex-row gap-6 items-start mb-6">
          <div className="flex items-center gap-4">
            <svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <path d="M23.816 32.8496V44.8496C23.816 45.2032 23.6756 45.5424 23.4255 45.7924C23.1755 46.0425 22.8363 46.1829 22.4827 46.1829H17.1494C16.7958 46.1829 16.4566 46.0425 16.2066 45.7924C15.9565 45.5424 15.816 45.2032 15.816 44.8496V32.8496C15.816 32.496 15.9565 32.1568 16.2066 31.9068C16.4566 31.6568 16.7958 31.5163 17.1494 31.5163H22.4827C22.8363 31.5163 23.1755 31.6568 23.4255 31.9068C23.6756 32.1568 23.816 32.496 23.816 32.8496ZM33.1494 16.8496H27.816C27.4624 16.8496 27.1233 16.9901 26.8732 17.2401C26.6232 17.4902 26.4827 17.8293 26.4827 18.1829V44.8496C26.4827 45.2032 26.6232 45.5424 26.8732 45.7924C27.1233 46.0425 27.4624 46.1829 27.816 46.1829H33.1494C33.503 46.1829 33.8421 46.0425 34.0922 45.7924C34.3422 45.5424 34.4827 45.2032 34.4827 44.8496V18.1829C34.4827 17.8293 34.3422 17.4902 34.0922 17.2401C33.8421 16.9901 33.503 16.8496 33.1494 16.8496ZM43.816 23.5163H38.4827C38.1291 23.5163 37.7899 23.6568 37.5399 23.9068C37.2898 24.1568 37.1494 24.496 37.1494 24.8496V44.8496C37.1494 45.2032 37.2898 45.5424 37.5399 45.7924C37.7899 46.0425 38.1291 46.1829 38.4827 46.1829H43.816C44.1697 46.1829 44.5088 46.0425 44.7589 45.7924C45.0089 45.5424 45.1494 45.2032 45.1494 44.8496V24.8496C45.1494 24.496 45.0089 24.1568 44.7589 23.9068C44.5088 23.6568 44.1697 23.5163 43.816 23.5163Z" fill="#1F59EE" />
            </svg>
            <div>
              <h1 className="font-poppins text-[22px] font-semibold leading-[20px] text-[#1A1A1A]">Brand Manager</h1>
              <p className="font-inter text-sm font-medium leading-[18px] text-[#64748B] mt-1">Manage your brand partnerships with style</p>
            </div>
          </div>
          <button
        ...
        {/* Stats Cards */}
        <div className="grid gap-4 mb-6 [grid-template-columns:repeat(auto-fit,minmax(220px,1fr))]">
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <div className="flex items-center gap-3">
              <svg width="45" height="45" viewBox="0 0 45 45" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect width="45" height="45" rx="6" fill="#00813B" fillOpacity="0.08" />
                <path fillRule="evenodd" clipRule="evenodd" d="M33.4202 22.0651L33.9411 21.7619C34.5414 21.3404 34.9138 20.7784 34.9138 20.1781V17.9778L32.6944 12.6535C32.4465 12.0592 31.9697 11.6377 31.3172 11.6377H14.4596C13.8061 11.6377 13.3304 12.0592 13.0824 12.6535L10.8621 17.9778V20.1781C10.8621 22.2136 14.7767 23.3852 17.1508 21.7164C17.6828 21.3424 18.0572 20.83 18.0572 20.1781H19.2909C19.2909 22.2065 23.2148 23.3758 25.5134 21.7619C26.1136 21.3404 26.4861 20.7784 26.4861 20.1781H27.7197C27.7197 22.0974 31.1275 23.1647 33.4202 22.0651ZM20.4602 33.3616H25.3164V27.4551C25.3164 27.1408 25.0594 26.8821 24.7473 26.8821H21.0292C20.717 26.8821 20.4601 27.1408 20.4601 27.4551L20.4602 33.3616ZM22.8884 23.7601C24.5095 23.7601 26.1778 23.1395 27.0911 21.9499C28.3789 23.6216 31.1332 24.0946 33.1219 23.5003V32.0216C33.1219 32.7675 32.5066 33.3618 31.7658 33.3618H26.5501V27.4553C26.5501 26.4557 25.7401 25.6401 24.7473 25.6401H21.0292C20.0365 25.6401 19.2264 26.4557 19.2264 27.4553V33.3618H14.0097C13.269 33.3618 12.6536 32.7675 12.6536 32.0216L12.6546 23.5003C14.6441 24.0946 17.3985 23.6216 18.6854 21.9499C19.5989 23.1395 21.2674 23.7601 22.8884 23.7601Z" fill="#00813B" />
              </svg>
              <div>
                <div className="font-inter text-2xl font-bold leading-[20px] text-[#00813B]">{statistics.active}</div>
                <div className="font-poppins text-base font-medium leading-[20px] text-[#00813B] mt-2">Active Brands</div>
              </div>
            </div>
          </div>
          ...
          {/* <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-center gap-3">
              <svg width="45" height="45" viewBox="0 0 45 45" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect width="45" height="45" rx="6" fill="#0D4FE1" fillOpacity="0.08" />
                <path d="M29.4829 14.6872V15.4997L25.2283 13.2489C23.6178 12.4035 21.3704 12.4035 19.7719 13.2489L15.5174 15.5106V14.6872C15.5174 12.2169 16.9956 10.8555 19.6998 10.8555H25.3004C28.0046 10.8555 29.4829 12.2169 29.4829 14.6872Z" fill="#0D4FE1" />
                <path d="M29.5159 17.3719L29.3477 17.2882L27.7139 16.3558L24.3262 14.4073C23.293 13.8096 21.7072 13.8096 20.6741 14.4073L17.2863 16.3438L15.6525 17.3001L15.4363 17.4077C13.3339 18.8183 13.1898 19.0813 13.1898 21.3406V26.7438C13.1898 29.0031 13.3339 29.2661 15.4843 30.7125L20.6741 33.6891C21.1907 33.9999 21.8394 34.1314 22.5001 34.1314C23.1488 34.1314 23.8096 33.988 24.3262 33.6891L29.564 30.6767C31.6783 29.2661 31.8105 29.0151 31.8105 26.7438V21.3406C31.8105 19.0813 31.6663 18.8183 29.5159 17.3719ZM25.8519 23.9824L25.119 24.879C24.9989 25.0105 24.9148 25.2615 24.9268 25.4408L24.9989 26.5884C25.047 27.2937 24.5424 27.6523 23.8817 27.4013L22.8125 26.9709C22.6127 26.9112 22.3996 26.9112 22.1998 26.9709L21.1306 27.3893C20.4699 27.6523 19.9653 27.2817 20.0134 26.5765L20.0854 25.4289C20.0899 25.2249 20.0218 25.0259 19.8932 24.867L19.1484 23.9824C18.6919 23.4445 18.8961 22.8468 19.5809 22.6675L20.6981 22.3806C20.8783 22.3328 21.0826 22.1654 21.1787 22.022L21.8034 21.0657C22.1878 20.4679 22.8005 20.4679 23.1969 21.0657L23.8216 22.022C23.9177 22.1774 24.1339 22.3328 24.3021 22.3806L25.4194 22.6675C26.1041 22.8468 26.3084 23.4445 25.8519 23.9824Z" fill="#0D4FE1" />
              </svg>
              <div>
                <div className="font-inter text-2xl font-bold leading-[20px] text-[#0D4FE1]">{statistics.total}</div>
                <div className="font-poppins text-base font-medium leading-[20px] text-[#0D4FE1] mt-2">Total Brands</div>
              </div>
            </div>
          </div> */}
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <div className="flex items-center gap-3">
              <div className="w-[45px] h-[45px] flex items-center justify-center rounded-md bg-[#A55E00]/10">⏳</div>
              <div>
                <div className="font-inter text-2xl font-bold text-[#A55E00]">{reviewData?.counts?.pending || 0}</div>
                <div className="font-poppins text-base font-medium text-[#A55E00] mt-1">Pending Stores</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <div className="flex items-center gap-3">
              <div className="w-[45px] h-[45px] flex items-center justify-center rounded-md bg-[#00813B]/10">✅</div>
              <div>
                <div className="font-inter text-2xl font-bold text-[#00813B]">{reviewData?.counts?.approved || 0}</div>
                <div className="font-poppins text-base font-medium text-[#00813B] mt-1">Approved Stores</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <CustomDropdown
              ...
              options={[{ value: "", label: "All Brands" }, { value: "true", label: "Featured Only" }, { value: "false", label: "Non-Featured" }]}
            ...
            <div className="mt-3 pt-3 border-t border-gray-200 flex flex-wrap gap-2 items-center">
              {filters.search && (
                <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm flex items-center gap-1">
                  ...
                  <button onClick={() => { setSearchInput(""); handleFilterChange("search", ""); }} className="ml-1 hover:bg-blue-200 rounded-full p-0.5">✕</button>
                </span>
              )}
              {filters.category !== "All Brands" && (
                <span className="bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm flex items-center gap-1">
                  Category: {filters.category}
                  <button onClick={() => handleFilterChange("category", "All Brands")} className="ml-1 hover:bg-green-200 rounded-full p-0.5">✕</button>
                </span>
              )}
              {filters.isActive && (
                <span className="bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-sm flex items-center gap-1">
                  ...
                  <button onClick={() => handleFilterChange("isActive", null)} className="ml-1 hover:bg-yellow-200 rounded-full p-0.5">✕</button>
                </span>
              )}
              {filters.isFeature && (
                <span className="bg-purple-100 text-purple-800 px-3 py-1 rounded-full text-sm flex items-center gap-1">
                  Featured: {filters.isFeature === "true" ? "Yes" : "No"}
                  <button onClick={() => handleFilterChange("isFeature", null)} className="ml-1 hover:bg-purple-200 rounded-full p-0.5">✕</button>
                </span>
              )}
              <button onClick={clearAllFilters} className="ml-auto text-sm text-red-600 hover:text-red-700 font-medium">Clear All Filters</button>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-row justify-between items-start md:items-center mb-6 gap-4 md:gap-0">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 w-full md:w-auto">
            <h2 className="text-lg sm:text-xl font-semibold text-[#1A1A1A]">Brands List</h2>
          ...
          <div className="flex flex-row sm:flex-row items-center gap-2 w-full sm:w-auto mt-3 md:mt-0">
            ...
            <CustomDropdown
              ...
              options={[{ value: 4, label: 4 }, { value: 8, label: 8 }, { value: 12, label: 12 }, { value: 24, label: 24 }, { value: 48, label: 48 }]}
        ...
        <div className="relative">
          ...
          {brands.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
              {brands.map((brand) => {
                const pendingStore = pendingStoreByBrandId[brand.id];
                const isApprovingThis = approvingShop === pendingStore?.shop;
                    ...
                    <div className="p-5 flex-grow flex flex-col">
                      ...
                      {pendingStore && (
                        <div className="w-full flex justify-end mb-1">
                          <button
                            onClick={() => handleApproveStore(pendingStore.shop, brand.id)}
                            disabled={isApprovingThis || actionLoading}
                            className="shrink-0 inline-flex items-center gap-1 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold px-3 py-1 rounded-full transition-colors disabled:opacity-60"
                          >
                            {isApprovingThis ? (
        ...
        {pagination.totalPages > 1 && (
          <div className="mt-8 flex flex-col sm:flex-row justify-between items-center gap-4">
            <div className="text-sm text-gray-600">
              ...
              {" "}to{" "}
              <span className="font-semibold">{Math.min(pagination.currentPage * pagination.itemsPerPage, pagination.totalItems)}</span>
