...
const BrandPartnerSchema = z.object({
  ...
  contractStart: z.string().refine(date => !isNaN(Date.parse(date)), "Invalid contract start date"),
  contractEnd: z.string().refine(date => !isNaN(Date.parse(date)), "Invalid contract end date"),
  goLiveDate: z.string().refine(date => !isNaN(Date.parse(date)), "Invalid go live date").optional(),
    ...
    try {
        ...
        if (parsedData.logoFile && parsedData.logoFile.size > 0) {
            try {
                ...
                const filename = `${validatedData.brandName.toLowerCase().replace(/\s+/g, '_')}_${timestamp}.${extension}`;
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            await tx.brandTerms.create({
                data: {
                    ...
                    goLiveDate: validatedData.goLiveDate ? new Date(validatedData.goLiveDate) : new Date(),
            ...
            await tx.vouchers.create({
                data: {
                    ...
                    denominationValue: validatedData.denominationValue,
...
export async function getBrandPartnerDashboard(brandId) {
    try {
        ...
        const daysUntilExpiry = contractEndDate ? 
            Math.ceil((contractEndDate - currentDate) / (1000 * 60 * 60 * 24)) : null;
        ...
        const dashboard = {
            ...
            metrics: {
                ...
                activeIntegrations: brandPartner.integrations.filter(i => i.isActive).length,
                totalContacts: brandPartner.brandcontacts.length,
                daysUntilContractExpiry: daysUntilExpiry
            },
            status: {
                contractStatus: daysUntilExpiry > 30 ? 'active' : daysUntilExpiry > 0 ? 'expiring_soon' : 'expired',
                integrationStatus: brandPartner.integrations.length > 0 ? 'configured' : 'pending',
                bankingStatus: brandPartner.brandBankings.length > 0 ? 'verified' : 'pending'
            }
        };
...
export async function toggleBrandPartnerStatus(brandId, status) {
    try {
        ...
        return {
            success: true,
            message: `Brand partner ${status ? 'activated' : 'deactivated'} successfully`,
            data: updatedBrand,
            status: 200
        };
